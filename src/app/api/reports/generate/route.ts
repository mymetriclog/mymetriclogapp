...
export async function POST(req: NextRequest) {
  try {
    ...
    const { reportType, dateRange, latitude, longitude } = await req.json();
    ...
    const [gmailData, googleCalendarData, fitbitData, spotifyData] =
      await Promise.all([
        fetchIntegrationData("gmail", session.user.id),
        fetchIntegrationData("google-calendar", session.user.id),
        fetchIntegrationData("fitbit", session.user.id),
        fetchIntegrationData("spotify", session.user.id),
      ]);
